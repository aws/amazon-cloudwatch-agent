...
func checkDatum(
	...
	// baseline-instrument: log observed elapsed when close to the 1s budget so we
	// can see how the loop's cumulative time correlates with GOMAXPROCS/goroutine load.
	if elapsed > 500*time.Millisecond {
		t.Logf("baseline-instrument: checkDatum elapsed=%s (budget 1s) numMetrics=%d unit=%s goroutines=%d GOMAXPROCS=%d",
			elapsed, numMetrics, unit, runtime.NumGoroutine(), runtime.GOMAXPROCS(0))
	}
