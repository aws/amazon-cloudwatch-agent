...
func TestMiddleware(t *testing.T) {
	...
	ctx := context.Background()
	// baseline-instrument: track first-invocation timestamps for each handler phase.
	// atomic.CompareAndSwap-Nano so multiple concurrent PutMetricData responses don't clobber each
	// other; we only care about the FIRST time each phase fired. Assertion logic below unchanged.
	var reqAt, respAt int64
	...
	handler.On("HandleRequest", mock.Anything, mock.Anything).Run(func(mock.Arguments) {
		atomic.CompareAndSwapInt64(&reqAt, 0, time.Now().UnixNano())
	})
	handler.On("HandleResponse", mock.Anything, mock.Anything).Run(func(mock.Arguments) {
		atomic.CompareAndSwapInt64(&respAt, 0, time.Now().UnixNano())
	})
	...
	time.Sleep(2*time.Second + 2*cw.config.ForceFlushInterval)
	sleepEnd := time.Now()
	// baseline-instrument: log the observed timeline BEFORE the AssertCalled fires so we see the
	// data even when AssertCalled subsequently fails.
	sinceConsume := func(unix int64) string {
		if unix == 0 {
			return "NEVER FIRED"
		}
		return time.Unix(0, unix).Sub(consumeStart).String()
	}
	t.Logf("baseline-instrument: TestMiddleware sleep_req=%s sleep_actual=%s "+
		"first_HandleRequest=%s first_HandleResponse=%s numCalls=%d goroutines=%d GOMAXPROCS=%d",
		2*time.Second+2*cw.config.ForceFlushInterval,
		sleepEnd.Sub(sleepStart),
		sinceConsume(atomic.LoadInt64(&reqAt)),
		sinceConsume(atomic.LoadInt64(&respAt)),
		len(handler.Calls), runtime.NumGoroutine(), runtime.GOMAXPROCS(0))
...
func TestBackoffRetries(t *testing.T) {
	...
	for i := 0; i <= defaultRetryCount; i++ {
		...
		// baseline-instrument: unconditional per-iter log so we see distribution of actual sleep
		// durations vs the [sleeps[i]/2, sleeps[i]+leniency] window on every run.
		t.Logf("baseline-instrument: backoff iter=%d target=%s expected_range=[%s,%s] actual=%s goroutines=%d",
			i, sleeps[i], sleeps[i]/2, sleeps[i]+leniency, elapsed, runtime.NumGoroutine())
