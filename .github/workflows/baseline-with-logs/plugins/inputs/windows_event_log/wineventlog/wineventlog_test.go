...
func assertStateFileRange(t *testing.T, fileName string, rl state.RangeList) {
	...
	// baseline-instrument: capture what the state file actually held at the single 200ms read.
	// On a Windows flake this shows content="" (or a stale/partial range) -> the async state
	// flush (saveStateInterval=100ms) hadn't landed within the fixed 200ms window.
	t.Logf("baseline-instrument: assertStateFileRange file=%s want=%q got=%q len=%d goroutines=%d GOMAXPROCS=%d",
		fileName, marshalRangeList(rl), string(content), len(content), runtime.NumGoroutine(), runtime.GOMAXPROCS(0))
