...
func TestTailerSrc(t *testing.T) {
	...
	tailFileDur := time.Since(beforeCall)

	require.NoError(t, err, fmt.Sprintf("Failed to create tailer src for file %v with error: %v", file, err))
	// baseline-instrument: log the counter transition before the assertion. This is a shared
	// process-global atomic; the assertion below is racy if another test's tailer cleanup
	// goroutine decrements between beforeCount and Load(). We want to see exactly what values
	// were observed when the flake fires.
	observed := tail.OpenFileCount.Load()
	t.Logf("baseline-instrument: TestTailerSrc OpenFileCount before=%d after_TailFile=%d expected=%d "+
		"TailFile_call_took=%s goroutines=%d",
		beforeCount, observed, beforeCount+1, tailFileDur, runtime.NumGoroutine())
