...
func TestLogsFileRemove(t *testing.T) {
	...
	case <-time.After(1 * time.Second):
		// baseline-instrument: dump the observable state on timeout so we can
		// see how far along the shutdown pipeline the tailer got when the
		// grace window expired. On Windows CI under contention the polling
		// watcher's read-failure-detects-deletion path can take longer than
		// 1s to propagate all the way through runTail (observed in run
		// 30110009561 baseline iter 1).
		select {
		case <-deleteRequested:
			t.Logf("baseline-instrument: TestLogsFileRemove TIMEOUT elapsed=%s "+
				"delete_fired=true stopped_closed=false goroutines=%d GOMAXPROCS=%d",
				time.Since(testStart), runtime.NumGoroutine(), runtime.GOMAXPROCS(0))
		default:
			t.Logf("baseline-instrument: TestLogsFileRemove TIMEOUT elapsed=%s "+
				"delete_fired=false (delete goroutine hadn't run yet!) stopped_closed=false goroutines=%d GOMAXPROCS=%d",
				time.Since(testStart), runtime.NumGoroutine(), runtime.GOMAXPROCS(0))
		}
		t.Errorf("tailerSrc should have stopped after tile is removed")
	case <-stopped:
		t.Logf("baseline-instrument: TestLogsFileRemove PASS elapsed=%s goroutines=%d",
			time.Since(testStart), runtime.NumGoroutine())
	}
