...
func TestExistingAttributesNotOverwritten(t *testing.T) {
	...
	// baseline-instrument: dispatch a probe metric to observe exactly which
	// async-loaded attribute sources have (or haven't) completed by the
	// 1-second mark. When the enclosing test fails on Windows CI it's
	// because one or more of ImageId / tagKey2 / VolumeId still hadn't been
	// populated -- this log lets us see the state at the boundary.
	probeMd := createTestMetrics([]map[string]string{{
		"InstanceId": "i-100000",
		"device":     device1,
	}})
	probeOut, _ := tagger.processMetrics(context.Background(), probeMd)
	hasImg, hasTag, hasVol, probeMetricsLen := false, false, false, 0
	if probeOut.ResourceMetrics().Len() > 0 &&
		probeOut.ResourceMetrics().At(0).ScopeMetrics().Len() > 0 &&
		probeOut.ResourceMetrics().At(0).ScopeMetrics().At(0).Metrics().Len() > 0 {
		probeMetricsLen = probeOut.ResourceMetrics().At(0).ScopeMetrics().At(0).Metrics().Len()
		attrs := probeOut.ResourceMetrics().At(0).ScopeMetrics().At(0).Metrics().At(0).Gauge().DataPoints().At(0).Attributes()
		_, hasImg = attrs.Get("ImageId")
		_, hasTag = attrs.Get(tagKey2)
		_, hasVol = attrs.Get("VolumeId")
	}
	t.Logf("baseline-instrument: TestExistingAttributesNotOverwritten at 1s "+
		"probe_metrics=%d hasImageId=%v hasTagKey2=%v hasVolumeId=%v goroutines=%d GOMAXPROCS=%d",
		probeMetricsLen, hasImg, hasTag, hasVol, runtime.NumGoroutine(), runtime.GOMAXPROCS(0))
