...
func TestAdmitAndRollup(t *testing.T) {
	...
	// baseline-instrument: reveal which random RemoteOperation values got picked
	// AND what Admit returned each iter. The bug (topKMetrics.Push initializing
	// minMetric from oldMetric with freq=0) causes the 3rd distinct random key
	// to evict an earlier entry inside Push, and any subsequent draw of that
	// same key returns admitted=true via `found` -- producing a 3-entry
	// admittedAttributes when Threshold=2 promises at most 2. Log the exact
	// per-iter timeline so the fix is provable.
	...
	for i := 0; i < 10; i++ {
		...
		t.Logf("baseline-instrument: TestAdmitAndRollup iter=%d key=%s admitted=%v uniqueAdmittedSoFar=%d",
			i, uniqKey.AsString(), ok, len(admittedAttributes))
	}
	t.Logf("baseline-instrument: TestAdmitAndRollup final admittedAttributes keys=%v (threshold=2)",
		mapKeys(admittedAttributes))
	assert.Equal(t, 2, len(admittedAttributes), fmt.Sprintf("admitted attributes are %v", admittedAttributes))
}

// baseline-instrument helpers -- keep at file scope so they compile regardless
// of test iteration ordering. They exist ONLY in the instrumented baseline
// overlay; the origin/main file has neither. They must NOT change any
// production or test semantics; both are pure functions over local data.
func mapKeys(m map[string]pcommon.Map) []string {
