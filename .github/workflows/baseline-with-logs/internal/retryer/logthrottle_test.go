...
func TestLogThrottleRetryerLogging(t *testing.T) {
	...
	r.Stop()
	// baseline-instrument: capture the pre-shutdown log-buffer state so we can
	// see whether events made it to the watcher's log slices at all. The
	// original code raced the watcher goroutine at shutdown (throttleChan
	// buffer=1 + non-blocking send + non-draining shutdown) which allowed
	// events queued between the last iteration and Stop() to be silently
	// dropped under Windows CI contention.
	preSleepDebugs := len(l.debugs)
	preSleepInfos := len(l.infos)
	time.Sleep(200 * time.Millisecond) // Wait a bit to collect all logs
	postSleepDebugs := len(l.debugs)
	postSleepInfos := len(l.infos)
	t.Logf("baseline-instrument: TestLogThrottleRetryerLogging expected=%d "+
		"preSleep{debugs=%d,infos=%d} postSleep{debugs=%d,infos=%d} goroutines=%d GOMAXPROCS=%d",
		totalThrottleCnt, preSleepDebugs, preSleepInfos, postSleepDebugs, postSleepInfos,
