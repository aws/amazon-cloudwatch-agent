...
func TestTimedDeleterWithIDCheck_DeleteWithDelay_NoUpdate(t *testing.T) {
	...
	_, stillPresent := m.Load(key)
	t.Logf("baseline-instrument: NoUpdate delay=10ms sleep_req=20ms "+
		"call_took=%s sleep_actual=%s stillPresent=%v total_since_call=%s goroutines=%d GOMAXPROCS=%d",
		callReturn.Sub(callStart), sleepEnd.Sub(sleepStart), stillPresent,
		time.Since(callStart), runtime.NumGoroutine(), runtime.GOMAXPROCS(0))
...
func TestTimedDeleterWithIDCheck_DeleteWithDelay_WithUpdate(t *testing.T) {
	...
	_, stillPresent := m.Load(key)
	t.Logf("baseline-instrument: WithUpdate delay=20ms first_sleep=10ms second_sleep=20ms "+
		"pre_update=%s update_took=%s total_since_call=%s stillPresent=%v goroutines=%d",
		preUpdate.Sub(callStart), postUpdate.Sub(preUpdate),
		sleepEnd.Sub(callStart), stillPresent, runtime.NumGoroutine())
...
func TestTimedDeleterWithIDCheck_DeleteWithDelay_InvalidType(t *testing.T) {
	...
	time.Sleep(20 * time.Millisecond)
	_, stillPresent := m.Load(key)
	t.Logf("baseline-instrument: InvalidType delay=10ms sleep=20ms total_since_call=%s "+
		"stillPresent=%v goroutines=%d",
		time.Since(callStart), stillPresent, runtime.NumGoroutine())
	if _, ok := m.Load(key); !ok {
