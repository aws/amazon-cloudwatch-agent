...
Refer
to [public docs](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/deploy_servicelens_CloudWatch_agent_deploy_ECS.html#deploy_servicelens_CloudWatch_agent_deploy_ECS_roles)
on what permissions are required.

The three main AWS-managed policies to attach to the execution role (role assumed by ECS) are:

* AmazonSSMReadOnlyAccess
* AmazonECSTaskExecutionRolePolicy
* CloudWatchAgentServerPolicy

The main AWS-managed policy to attach to the task role (role used by the tasks while running) is:

* CloudWatchAgentServerPolicy

**This will likely be evolved over time.**

### Create a test S3 bucket

See [docs](https://docs.aws.amazon.com/AmazonS3/latest/userguide/create-bucket-overview.html). The bucket does **NOT**
require public access. It is used as a placeholder for the build artifacts from building CloudWatch agent, so that each
build can be used during the integration tests.

### EC2 integration test setup
...
The security group(s) that the EC2 integration tests use should include the following for ingress:
...
on creating the key pair. This key pair is used for the Terraform user to get onto the hosts to execute the integration
tests natively on each instance
...
### ECS integration test setup

#### Create a private ECR repository
Create a repository in your AWS account, named `cwagent-integration-test`. 
The [public docs](https://docs.aws.amazon.com/AmazonECR/latest/userguide/getting-started-console.html)
are a good place to start for creating that repository. Note that the repository does not require
KMS encryption, and is not required to be public. 
...
Navigate to the localstack terraform directory, initialize Terraform and apply the tf plan. For example, to run EC2
