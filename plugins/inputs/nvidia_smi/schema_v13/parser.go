...
func Parse(acc telegraf.Accumulator, buf []byte) error {
	...
	for i := range s.Gpu {
		...
		// v13 adds detailed ECC error breakdowns (volatile/aggregate, SRAM source attribution)
		// that are absent from the v12 schema.
		...
		// v13 renames the remapped-row fields; reference the new struct field names directly.
