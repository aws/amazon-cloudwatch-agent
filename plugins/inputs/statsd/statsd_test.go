...
func TestParse_Timings(t *testing.T) {
	...
	metric := metrics[0]
	assert.Equal(t, "test_timing", metric.Measurement)
	fields := metric.Fields
	assert.Equal(t, 1, len(fields))
	assert.Equal(t, dist, fields[defaultFieldName])
}
...
func TestParse_DataDogTags(t *testing.T) {
	...
	for statName, tags := range testTags {
		for k, v := range tags {
			...
			if sourceTags[statName][k] != v && !(sourceTags[statName][k] == "<empty>" && v == "") {
...
func TestParse_MeasurementsWithMultipleValues(t *testing.T) {
	...
	if cachedtiming, ok := s_single.timings["metric_type=timingvalid_multiple"]; !ok {
		...
		dist := cachedtiming.fields[defaultFieldName].(distribution.Distribution)
		sampleCount := dist.SampleCount()
		if math.Abs(sampleCount-12) >= 1 {
			t.Errorf("Expected 11 additions, got %f", sampleCount)
		}

		max := dist.Maximum()
		if max != 1 {
			t.Errorf("Expected max input to be 1, got %f", max)
		}
	}
...
func TestParse_Timings_MultipleFieldsWithTemplate(t *testing.T) {
	...
	assert.Equal(t, dist, fields["success"])
	assert.Equal(t, dist2, fields["error"])
}
...
func TestParse_Timings_MultipleFieldsWithoutTemplate(t *testing.T) {
	...
	if metrics[0].Measurement == "test_timing_success" {
		metric = metrics[0]
	} else {
		metric = metrics[1]
	}
	...
	if metrics[0].Measurement == "test_timing_error" {
		metric = metrics[0]
	} else {
		metric = metrics[1]
	}
	...
	fields = metric.Fields
	assert.Equal(t, 1, len(fields))
	assert.Equal(t, dist2, fields[defaultFieldName])
}
