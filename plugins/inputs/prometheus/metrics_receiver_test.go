...
func Test_metricAppender_Add_BadMetricName(t *testing.T) {
	...
	ls := labels.FromStrings("name_a", "value_a", "name_b", "value_b")
...
func Test_metricAppender_Add(t *testing.T) {
	...
	var ts int64 = 10
	var v = 10.0
	ls := labels.FromStrings("__name__", "metric_name", "tag_a", "a")
...
func Test_loadConfigFromFileWithTargetAllocator(t *testing.T) {
	...
	assert.Equal(t, taManager.config.TargetAllocator.Get().CollectorID, "collector-1")
	assert.Equal(t, taManager.config.TargetAllocator.Get().TLS.CAFile, DefaultTLSCaFilePath)
}
