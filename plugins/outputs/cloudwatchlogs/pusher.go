	...
	for {
		...
		case *cloudwatchlogs.InvalidSequenceTokenException:
			if p.sequenceToken == nil {
				p.Log.Infof("First time sending logs to %v/%v since last restart so sequenceToken is nil, learned new token:(%v): %v",  p.Group, p.Stream, e.ExpectedSequenceToken, e.Message())
			} else {
