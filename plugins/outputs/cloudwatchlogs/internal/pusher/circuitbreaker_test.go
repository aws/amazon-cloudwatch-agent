...
func TestCircuitBreakerBlocksTargetAfterFailure(t *testing.T) {
	...
	service := &stubLogsService{
		ple: func(_ context.Context, input *cloudwatchlogs.PutLogEventsInput) (*cloudwatchlogs.PutLogEventsOutput, error) {
...
func TestTerminalFailureResumesHaltedTarget(t *testing.T) {
	...
	service := okStubService(func(context.Context, *cloudwatchlogs.PutLogEventsInput) (*cloudwatchlogs.PutLogEventsOutput, error) {
		switch calls.Add(1) {
		case 1:
			return nil, &types.ServiceUnavailableException{} // retryable -> heap -> fail() -> halt
		case 2:
			return nil, &types.InvalidParameterException{} // terminal -> updateState, no resume
		default:
			delivered.Add(1)
			return &cloudwatchlogs.PutLogEventsOutput{}, nil
		}
