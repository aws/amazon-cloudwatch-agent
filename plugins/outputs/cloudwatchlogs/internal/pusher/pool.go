...
type WorkerPool interface {
	// Submit reports whether the task was accepted. False means the task is not guaranteed
	// to run, so the caller must finalize whatever it owns (for a send, the batch).
	// Finalization is idempotent, so if a draining worker still ran the task the caller's
	// finalize is a harmless no-op: the first finalizer wins.
...
func (p *workerPool) Submit(task func()) bool {
	...
	select {
	case p.tasks <- task:
		// The guard above races with Stop: it can observe stopping==false, then Stop can run
		// to completion (workers drain the buffer, hit the default branch, and exit) before
		// this send executes. Because select chooses randomly among ready cases, the send can
		// still win over the closed stopCh and buffer a task with no worker left to run it.
		// Re-check here: if stopping is now set this send lost the race, so report not-accepted
		// and let the caller finalize the batch. A task enqueued before Stop closed stopCh is
		// still drained by worker(), so any task reported accepted is guaranteed to run.
		if p.stopping.Load() {
			return false
		}
		return true
