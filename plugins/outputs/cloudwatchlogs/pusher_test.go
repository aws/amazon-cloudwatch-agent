...
func TestRetryWaitShort(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(fmt.Sprintf("%d", tt.n), func(t *testing.T) {
			for _ = range 1000 {
				duration := retryWaitShort(tt.n)
				assert.GreaterOrEqual(t, duration, tt.minDuration, "retryWait(%v) should be greater than or equal to %v", tt.n, tt.minDuration)
				assert.LessOrEqual(t, duration, tt.maxDuration, "retryWait(%v) should be less than or equal to %v", tt.n, tt.maxDuration)
			}
		})
	}
}
