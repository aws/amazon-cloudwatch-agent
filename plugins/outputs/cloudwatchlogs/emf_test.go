...
func TestEMFHeaderPreservedOnRetriedBatch(t *testing.T) {
	...
	require.True(t, dest.isEMF.Load(), "EMF-shaped message must switch the dest to EMF mode")
...
func TestEMFHeaderNotLeakedToOtherDestinationsOnRetry(t *testing.T) {
	...
	require.True(t, emfDest.isEMF.Load())
	require.False(t, plainDest.isEMF.Load(), "a plain message must not switch the dest to EMF")
...
func TestEMFHeaderPreservedOnRetriedBatchSynchronous(t *testing.T) {
	...
	require.True(t, dest.isEMF.Load())
