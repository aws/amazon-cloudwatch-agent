...
func (agent *Agent) updateEffectiveConfig(
	...
	if newStatus.EffectiveConfig != nil {
		if newStatus.EffectiveConfig.ConfigMap != nil {
			...
			// Convert to string for displaying purposes with proper separation.
			agent.EffectiveConfig = ""
			for name, cfg := range newStatus.EffectiveConfig.ConfigMap.ConfigMap {
				if agent.EffectiveConfig != "" {
					agent.EffectiveConfig += "\n\n" + strings.Repeat("=", 80) + "\n"
				}
				if name != "" {
					agent.EffectiveConfig += "Config: " + name + "\n"
					agent.EffectiveConfig += strings.Repeat("-", 40) + "\n"
				}
