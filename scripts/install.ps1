...
# --- validate ---
if ($Cloud -notin @('aws', 'azure', 'gcp')) {
    Write-Error "unsupported cloud '${Cloud}' (expected: aws, azure, gcp)"
}
if ($Cloud -in @('azure', 'gcp')) {
    if (-not $RoleArn) {
        Write-Error "CWAGENT_AWS_ROLE_ARN is required for ${Cloud} cloud"
    }
    if (-not $Region) {
        Write-Error "CWAGENT_AWS_REGION is required for ${Cloud} cloud"
    }
}
