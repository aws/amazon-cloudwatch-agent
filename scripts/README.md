...
| `CWAGENT_AWS_ROLE_ARN`                  | IAM role ARN. `aws/setup.sh` derives the role name from it and fails when its partition or account does not match the shell's credentials. When set, the cloud-side setup script (`azure/setup.sh`, `gcp/setup.sh`) also installs |
