...
setup_gcp_gce() {
     ...
     # One describe reads the identity and the OS (which install payload): the
     # boot disk's license URIs name the image family, so a Windows instance is
     # recognized by the "windows" token in them.
     # gcloud's own stderr is not suppressed: a permission or throttling failure
     ...
     case "${VM_LICENSES}" in
     *[Ww]indows*) VM_OS="Windows" ;;
     *) VM_OS="Linux" ;;
     esac
     if [ -z "${SA_EMAIL}" ]; then
          die "no service account attached to '${INSTANCE_NAME}'. Attach one (the instance must be stopped first):
  gcloud compute instances set-service-account ${INSTANCE_NAME} --zone ${LOCATION} --service-account <sa-email>"
     ...
     # account may live in a different project than the VM. Its stderr is not
     # suppressed, for the same reason as the instance probe above.
     SA_UNIQUE_ID=$(gcloud iam service-accounts describe "${SA_EMAIL}" \
          --format 'value(uniqueId)') ||
          die "cannot read service account ${SA_EMAIL} (see the gcloud error above; needs iam.serviceAccounts.get on its project)"
     ...
     if [ -z "${ROLE_ARN}" ]; then
          print_await_arn "Service account unique ID (for the AWS setup): ${SA_UNIQUE_ID}"
          return
     fi
...
setup_gcp_gke() {
     ...
     if [ -z "${ROLE_ARN}" ]; then
          print_await_arn "OIDC issuer (for the AWS setup): ${GCP_OIDC_ISSUER}"
          return
     fi
     ...
     log "OIDC issuer (for the AWS setup): ${GCP_OIDC_ISSUER}"
}
