...
func TestContainerInsightsKarpenterConfig(t *testing.T) {
	resetContext(t)
	context.CurrentContext().SetMode(config.ModeEC2)
	context.CurrentContext().SetKubernetesMode(config.ModeEKS)
	...
	translator.SetTargetPlatform("linux")
	var input interface{}
	blob, err := os.ReadFile("./sampleConfig/opentelemetry/container_insights_karpenter_config.json")
	require.NoError(t, err)
	require.NoError(t, json.Unmarshal(blob, &input))
	_, _ = cmdutil.TranslateJsonMapToTomlConfig(input)

	var expected interface{}
	bs, err := os.ReadFile("./sampleConfig/opentelemetry/container_insights_karpenter_config.yaml")
	require.NoError(t, err)
	require.NoError(t, yaml.Unmarshal(bs, &expected))
