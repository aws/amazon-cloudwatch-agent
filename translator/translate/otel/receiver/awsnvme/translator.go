...
func (t *translator) Translate(conf *confmap.Conf) (component.Config, error) {
	...
	// Configure metrics based on measurement list
	c := confmap.NewFromStringMap(map[string]any{
		"metrics": getEnabledMeasurements(conf),
	})

	if err := c.Unmarshal(&cfg); err != nil {
		return nil, fmt.Errorf("unable to unmarshal unified nvme receiver (%s): %w", t.ID(), err)
	}
...
func getEnabledMeasurements(conf *confmap.Conf) map[string]any {
	...
	for _, m := range measurements {
		...
		// Include both EBS and Instance Store metrics
