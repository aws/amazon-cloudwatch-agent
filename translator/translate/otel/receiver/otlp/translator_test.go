...
func TestTracesTranslator(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"WithHTTPDefault": {
			...
			want: func(cfg *otlpreceiver.Config) bool {
				return cfg.HTTP.HasValue() && cfg.HTTP.Get().ServerConfig.NetAddr.Endpoint == "127.0.0.1:4318" && !cfg.GRPC.HasValue()
			},
		},
		"WithTLS": {
			config: EndpointConfig{
...
func TestMetricsTranslator(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"WithHTTPEndpoint": {
			...
			want: func(cfg *otlpreceiver.Config) bool {
				return cfg.HTTP.HasValue() && cfg.HTTP.Get().ServerConfig.NetAddr.Endpoint == "127.0.0.1:2345"
			},
		},
	}
