		...
		for i := 0; i < md.ResourceMetrics().Len(); i++ {
			...
			for j := 0; j < rm.ScopeMetrics().Len(); j++ {
				...
				for k := 0; k < ms.Len(); k++ {
					...
					for d := 0; d < dps.Len(); d++ {
						dp := dps.At(d)
						core, ok := dp.Attributes().Get("neuroncore")
						require.True(t, ok, "datapoint is missing neuroncore")
...
func TestNeuronMultiRuntimeKeepsEveryCorePerRuntime(t *testing.T) {
	...
	require.NoError(t, chain.ConsumeMetrics(context.Background(), multiRuntimeMetrics()))
...
// TestNeuronMultiRuntimeKeepsUntaggedDatapoints covers the metrics that carry no
// runtime_tag at all. `filter/cw_k8s_ci_v0_neuron` keeps the whole `^(neuron|...)`
// family, and the device- and hardware-level members of it (neuron_hardware_info,
// neurondevice_hw_ecc_events_total) are reported per device rather than per runtime.
// Adding runtime_tag to the groupbyattrs keys must not strand them: they belong in
// their own resource, which carries no aws.neuron.runtime.tag rather than inheriting
// some arbitrary runtime's. This is what the `where ... != nil` guards on the promote
// buy, and nothing else here exercises them.
func TestNeuronMultiRuntimeKeepsUntaggedDatapoints(t *testing.T) {
	processors := renderedNeuronProcessors(t)
	chain, sink := buildChain(t,
		processorConfig(t, processors, neuronGroupByAttrs),
		processorConfig(t, processors, neuronPromote))

	md := multiRuntimeMetrics()
	// A device-level metric from the same pod, with no runtime_tag.
	m := md.ResourceMetrics().At(0).ScopeMetrics().At(0).Metrics().AppendEmpty()
	m.SetName("neurondevice_hw_ecc_events_total")
	dp := m.SetEmptyGauge().DataPoints().AppendEmpty()
	dp.SetDoubleValue(3)
	a := dp.Attributes()
	a.PutStr("neurondevice", "0")
	a.PutStr("k8s.pod.name", testPod)
	a.PutStr("k8s.namespace.name", testNamespace)
	a.PutStr("k8s.container.name", testContainer)
	...
	for _, out := range sink.AllMetrics() {
		for i := 0; i < out.ResourceMetrics().Len(); i++ {
			...
			_, hasTag := rm.Resource().Attributes().Get("aws.neuron.runtime.tag")
			for j := 0; j < rm.ScopeMetrics().Len(); j++ {
				ms := rm.ScopeMetrics().At(j).Metrics()
				for k := 0; k < ms.Len(); k++ {
					dps := ms.At(k).Gauge().DataPoints()
					for d := 0; d < dps.Len(); d++ {
						if ms.At(k).Name() == "neurondevice_hw_ecc_events_total" {
							assert.False(t, hasTag,
								"a datapoint with no runtime_tag must not inherit another "+
									"runtime's tag from the resource it was grouped into")
							assert.Equal(t, float64(3), dps.At(d).DoubleValue())
							untagged++
							continue
						}
						assert.True(t, hasTag, "per-core datapoints must keep their runtime tag")
						tagged++
					}
				}
			}
	...
	assert.Equal(t, 1, untagged, "the untagged datapoint must still reach the exporter")
	assert.Equal(t, 4, tagged, "the four per-core datapoints must be unaffected")
}
