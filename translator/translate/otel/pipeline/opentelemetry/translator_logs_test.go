	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			if tc.wantErr != nil {
				...
				assert.Equal(t, "groupbyattrs/logs", got.Processors.Keys()[3].String())
				assert.Equal(t, "attributestocontext/opentelemetry", got.Processors.Keys()[4].String())
				assert.Equal(t, "transform/logs_cleanup", got.Processors.Keys()[5].String())
