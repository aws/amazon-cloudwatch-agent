...
func (t *translator) Translate(conf *confmap.Conf) (*common.ComponentTranslators, error) {
	...
	if highFrequencyGPUMetricsEnabled {
		...
		batchTranslator = batchprocessor.NewTranslator(common.WithName(t.pipelineName), batchprocessor.WithTelemetrySection(common.LogsKey), batchprocessor.WithTimeout(time.Minute))
	} else {
		// Use default timeout based on telemetry section
		batchTranslator = batchprocessor.NewTranslator(common.WithName(t.pipelineName), batchprocessor.WithTelemetrySection(common.LogsKey))
	}
