...
func TestTranslator(t *testing.T) {
	testCases := map[string]struct {
		...
		"TimeoutOverrideMetricsSection": {
			translator: NewTranslator(common.WithName("test"), WithTelemetrySection(common.MetricsKey), WithTimeout(45*time.Second)),
			input: map[string]interface{}{
				"metrics": map[string]interface{}{
					"force_flush_interval": 30,
				},
		...
		"TimeoutOverrideLogsSection": {
			translator: NewTranslator(common.WithName("test"), WithTelemetrySection(common.LogsKey), WithTimeout(15*time.Second)),
			input: map[string]interface{}{
				"logs": map[string]interface{}{},
			},
			want: &batchprocessor.Config{
		...
		"TimeoutOverrideNotConfiguredSection": {
			translator: NewTranslator(common.WithName("test"), WithTelemetrySection(common.TracesKey), WithTimeout(25*time.Second)),
			input: map[string]interface{}{
				"traces": map[string]interface{}{},
			},
			want: &batchprocessor.Config{
