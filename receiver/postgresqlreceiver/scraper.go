...
import (
	...
	"github.com/aws/amazon-cloudwatch-agent/receiver/postgresqlreceiver/internal/metadata"
	"github.com/aws/amazon-cloudwatch-agent/receiver/postgresqlreceiver/internal/priorityqueue"
)
...
func (p *postgreSQLScraper) scrape(ctx context.Context) (pmetric.Metrics, error) {
	...
	p.collectSessionStates(ctx, now, listClient, &errs) // CWA gap fix: session state metric
...
// CWA gap fix: session state breakdown metric
